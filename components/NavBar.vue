  ...
  <Menubar :model="items" breakpoint="700px">
    <template #end>
      <div class="flex gap-m">
        ...
        <Button :label="username" icon="pi pi-user" @click="toggleUserMenu" aria-haspopup="true"
          aria-controls="usermenu" outlined severity="info" />
        <Popover ref="usermenu">
          <div class="flex-column gap-l" style="width: 200px">
            <form class="flex-column gap-s" v-if="!authStore.hasAuth" @submit.prevent="login">
        ...
        <Button :icon="wsValues.icon" @click="toggleWsMenu" aria-haspopup="true" aria-controls="wsmenu"
          :outlined="wsValues.outlined" :severity="wsValues.severity" />
        <Popover ref="wsmenu">
...
const wsValues = computed(() => {
  ...
  if (wsStore.status === "connecting") return { severity: "danger", icon: "pi pi-sync", outlined: true }
  if (wsStore.status === "reconnect") return { severity: "danger", icon: "pi pi-sync", outlined: true }
